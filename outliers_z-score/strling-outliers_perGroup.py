...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='CSV file with sample groups (sample,group) where group is "case" or "control"')
    parser.add_argument(
...
def parse_groups(filename):
    """Parse groups file with columns sample, group"""
    groups = pd.read_csv(filename)
    # Verificar se as colunas estão corretas
    if not {'sample', 'group'}.issubset(groups.columns):
        raise ValueError('Groups file must contain columns "sample" and "group"')
    # Verificar se os grupos são válidos
    valid_groups = {'case', 'control'}
    invalid_groups = set(groups['group'].unique()) - valid_groups
    if invalid_groups:
        raise ValueError(f'Invalid groups found: {invalid_groups}. Valid groups are: {valid_groups}')
    return groups
...
def get_sample(fullpath):
    ...
    # Remove -genotype.txt ou -unplaced.txt preserving -1.bam
    sample = basename.replace('-genotype.txt', '').replace('-unplaced.txt', '')
    return sample
...
def parse_unplaced(filename):
    ...
    try:
        unplaced_counts = pd.read_csv(filename, sep=r'\s+', header = None,
...
def parse_genotypes(filename, min_clips = 5):
    ...
    sample_id = get_sample(filename)
    try:
        genotype_data = pd.read_csv(filename, sep=r'\s+', header = 0)
...
def parse_controls(control_file):
    ...
    control_estimates = pd.read_csv(control_file, index_col=0, sep=r'\s+', header = 0)
...
def z_score(x, df):
    ...
    frame with the columns 'mu' and 'sd' and index corresponding to the x values"""
    # Convert pandas Series to numpy arrays explicitly
    mu_array = df['mu'].values if hasattr(df['mu'], 'values') else np.array(df['mu'])
    sd_array = df['sd'].values if hasattr(df['sd'], 'values') else np.array(df['sd'])
    
    # Ensure x is a numpy array
    x_array = x.values if hasattr(x, 'values') else np.array(x)
    
    # Calculate z-scores with proper broadcasting
    z_scores = (x_array - mu_array[:, np.newaxis]) / sd_array[:, np.newaxis]
    
    # Convert back to DataFrame preserving original index and columns
    return pd.DataFrame(z_scores, index=x.index, columns=x.columns)
...
def main():
    ...
    # Check if all samples have group defined
    missing_groups = all_samples - set(groups['sample'])
    if missing_groups:
        sys.exit(f"ERROR: Samples missing from groups file: {missing_groups}")
    ...
    unplaced_long = pd.merge(unplaced_long, groups, on='sample', how='left')
    ...
    factor = 1 # scaling factor for log normalization
    ...
    # Add group information to genotype_data
    genotype_data = pd.merge(genotype_data, groups, on='sample', how='left')
